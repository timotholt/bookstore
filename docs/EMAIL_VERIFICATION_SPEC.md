...
4. After the recipient opens the link, require new password plus confirmation. Reuse a single password policy for signup and reset: minimum 6 characters, maximum 128 characters, and at least 3 of uppercase letters, lowercase letters, numbers, or non-alphanumeric symbols, matching Amazon’s published password guidance. Do not silently truncate, and use a common/breached-password blocklist using a local list or privacy-preserving lookup. Accept spaces, Unicode, and password-manager generated passwords. Existing passwords remain usable until changed.
